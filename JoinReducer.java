...
public class JoinReducer extends Reducer<TextPair, Text, Text, Text> {
  ...
  protected void reduce(TextPair key, Iterable<Text> values, Context context)
    ...
    while (iter.hasNext()) {
      ...
      String []name = stationName.toString().split("\t");
      String []mapperOutput = record.toString().split("\t");
      String value = metadata.getValue(mapperOutput[0]);
      String []movies;
      if (value != null){
    	  movies = value.split("\t");
    	  if(movies[1].matches("-?\\d+") && mapperOutput[3].equals("director") && Integer.parseInt(movies[1]) > 2010 && movies[2].contains("Western")){
        	  context.write(stationName, new Text(movies[0]+"\t"+movies[1]));
          }  
      }
